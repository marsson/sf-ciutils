...
function getSuccesDetailComponent(
  index: number,
  baseDeployResponse: Partial<MetadataApiDeployStatus>
): DeployMessage | undefined {
  if (baseDeployResponse.details ?? baseDeployResponse.details!.componentSuccesses === undefined) {
    return undefined;
  }

  let myReturn: DeployMessage | undefined;

  if (Array.isArray(baseDeployResponse.details!.componentSuccesses)) {
    myReturn = baseDeployResponse.details!.componentSuccesses[index];
  } else if (index === 0) {
    myReturn = baseDeployResponse.details!.componentSuccesses;
  }

  return myReturn;
}

const baseDeployResponse: Partial<MetadataApiDeployStatus> = {
  checkOnly: false,
...
export const getDeployResponse = (
  ...
  const response: MetadataApiDeployStatus = structuredClone({
    ...baseDeployResponse,
    ...overrides,
  ...
  if (type === 'failed') {
    ...
    response.details.componentSuccesses = structuredClone(
      getSuccesDetailComponent(0, baseDeployResponse)
    ) as DeployMessage;
    response.details.componentFailures = {
      ...(structuredClone(getSuccesDetailComponent(1, baseDeployResponse)) as DeployMessage),
      success: false,
      problemType: 'Error',
  ...
  if (type === 'failedTest') {
    ...
    response.details.componentFailures = structuredClone(
      getSuccesDetailComponent(1, baseDeployResponse)
    ) as DeployMessage;
    response.details.componentSuccesses = structuredClone(
      getSuccesDetailComponent(0, baseDeployResponse)
    ) as DeployMessage;
